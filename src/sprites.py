...
class GeoGebraUi(GeoGebraObject):
    ...
    def __init__(self,
        ...
        :param groups: 所属的精灵组
        '''
        super().__init__(*groups,pos=pos,z=z,name=name,image=image)
...
class InputBox(GeoGebraObject):
    ...
    def __init__(self, 
        ...
        super().__init__(*groups,pos=pos, z=z,image=pygame.Surface((0,0)),name="InputBox")
        self.can_dragged = False
        self.can_moved = False
        self.appear = False
...
class SelectBg(GeoGebraUi):
    ...
    def __init__(self,*groups,pos, height, z=DRAW_ORDER['Bg']):
        ...
        :param groups: 所属的精灵组
        '''
        image = pygame.Surface((WEIGH, 800 - height))
        image.fill(SELECT_BG_COLOR)
        logging.debug(pos)
        super().__init__(*groups, pos=pos,z=z,image=image, name="SelectBg")
        self.can_dragged = False
        self.sons = []
        self.create_son()
    ...
    def create_son(self):
        ...
        '''
        son_pos = pygame.Vector2(self.rect.topleft) + pygame.Vector2(10,0)
        for i in range(len(UI_TEXT)):
            ...
            son = GeoGebraUi(self.groups(), pos=son_pos, name=text, image=image)
            son.image = image
            son.text = text

            text_func = lambda obj: (obj.rect.centerx, obj.rect.centery + (obj.rect.height/2) + 10) 
            text_obj = Text(self.groups(),
                            ...
                            func=text_func,
                            parents=[son])
            text_obj.rect = text_obj.image.get_rect(center=text_func(son))
            ...
            if i == len(UI_TEXT) - 1:
                ...
                next_text_len = len(UI_TEXT[i+1])
            son_pos.x += len(text) / 2 + next_text_len + 75
...
class Dot(GeoGebraObject):
    ...
    def __init__(self,*groups, text:str, pos, type , parents = [], color = pygame.Color("blue"), z=DRAW_ORDER['Dot']):
        ...
        super().__init__(*groups,pos=pos-pygame.Vector2(DOT_RADIUS, DOT_RADIUS),  z=z, name=text, image=image)
...
class Text(GeoGebraObject):
    def __init__(self,*groups, text, pos, color, size, func, parents=None, z=DRAW_ORDER['Text']):
        ...
        super().__init__(*groups,pos=pos,parents=parents,z=z,image=image, name=text)
        self.rect = self.image.get_rect(topleft=pos)
        self.func = func
        if parents:
            self.can_dragged = False
...
class Line(GeoGebraObject):
    def __init__(self, *groups, pos, parents : list[Dot], color = pygame.Color("black"), z=DRAW_ORDER['Line']):
        ...
        self.get_x_y_img()
        super().__init__(*groups,pos=pos,parents=parents, z=z, name=self.parent1.name + self.parent2.name, image=self.image)
    ...
    def get_x_y_img(self):
        ...
        return [s_pos, e_pos, min_x, min_y, max_x, max_y]

    def rebuild(self, pos):

        s_pos, e_pos, min_x, min_y, max_x, max_y = self.get_x_y_img()

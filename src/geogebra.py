...
class GeoGebra:
    ...
    def __init__(self, screen:pygame.Surface):
        ...
        self.moved_bg = False
        self.all_sprites : Optional[GeoGebraObject] = CameraGroup(self.screen)
    ...
    def run(self, event:List[pygame.event.Event]) -> None:
        pos = self.get_mouse_pos()
        if event.type == pygame.KEYDOWN:
            logging.info(f"Key pressed: {event.key}")
            self.get_input(event)
            if event.key== pygame.K_SPACE:
                self.input_box.appear = True
    ...
    def draw(self):
        self.all_sprites.custom_draw(self.pos, self.select, self.get_mouse_pos(),
                                     True if self.choice == "显示/隐藏对象" else False)
    
    def choose_obj(self,
                   pos: pygame.Vector2,
                   event: List[pygame.event.Event]) -> None:
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            nearest_obj = self.search_nearest_object(pos, self.choose_obj_func)
            nearest_ui = self.search_nearest_object(pygame.mouse.get_pos() ,self.choose_ui_func)
            if nearest_obj and nearest_ui:
                logging.debug(f"Choose obj and ui is {nearest_obj.name}, {nearest_ui.name}")
            if nearest_ui:
    ...
    def get_input(self, event:pygame.event.Event):
        if self.input_box.appear:
            if self.input_box.get_rect().collidepoint(pygame.mouse.get_pos()):
        ...
        for obj in self.all_sprites:
            if match_condition(obj):
                if isinstance(obj, Line):
                    x1, y1 = obj.parent1.pos
                    x2, y2= obj.parent2.pos
                    ...
                    if dot <= 0:
                        # 垂足在A外侧，距离为P到A
                        distance = pygame.Vector2(ap_x, ap_y).length()

                    # AB长度平方
                    len_sq = ab_x ** 2 + ab_y ** 2
                    if dot >= len_sq:
                        # 垂足在B外侧，距离为P到B
                        distance = math.hypot(pos.x - x2, pos.y - y2)
                    ...
                    proj_x = x1 + t * ab_x
                    proj_y = y1 + t * ab_y
                    distance = math.hypot(pos.x - proj_x, pos.y - proj_y)
                else:
    ...
    def create_Dot(self, pos, type="normal"):
        '''
        创建一个点
        ...
        if obj1:
            if obj2:
                dot = Dot(self.all_sprites,
                          text=text,
                          pos=pos,
                          type=type if type else "intersection",
                          parents=[obj1, obj2],
    ...
    def create_LineFL(self, pos, long):
        ...
        dot2 = self.create_Dot(pos+pygame.Vector2(long, 0),type = "plot")
    ...
    def create_Line(self, pos):
        ...
        if dot2:
            for son in dot1.sons:
                if isinstance(son, Line):
                    for parent in son.parents:
                        if isinstance(parent, dot2):
                            return
            self.select_memory.pop()
    ...
    def save(self):
        ...
        for sprite in self.all_sprites:
            geogebra_map.append([sprite.name, sprite.pos])
...
class CameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self, pos:pygame.Vector2, select:GeoGebraObject, mouse_pos:pygame.Vector2, hide_things_appear):
        ...
        for layer in range(4):
            for sprite in sorted_sprites:
                ...
                sprite.draw(self.screen, pos, select, hide_things_appear)
